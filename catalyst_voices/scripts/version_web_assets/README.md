...
* Files that are generated when `flutter build --wasm` then `--wasm` flag of this script also needs to be set to true
...
* Use `--wasm=false` when building with JS-only output to skip wasm files that are genereated by `flutter build`
...
Options:
  ...
  -v, --[no-]verbose  Enable verbose logging
                      (defaults to on)
  -w, --wasm          Includes files that are generated by flutter when `flutter build --wasm`
                      (defaults to true)
