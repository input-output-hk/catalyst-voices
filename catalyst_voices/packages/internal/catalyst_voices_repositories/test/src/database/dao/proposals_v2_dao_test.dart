...
void main() {
  ...
  group(ProposalsV2Dao, () {
    for (final lang in CatalystDatabaseLanguage.values) {
      group('getProposalsBriefPage(${lang.name.toUpperCase()})', () {
        test('returns empty page for empty database', () async {
          // Given
          const request = PageRequest(page: 0, size: 10);

          // When
          final result = await dao.getProposalsBriefPage(request, lang: lang);
