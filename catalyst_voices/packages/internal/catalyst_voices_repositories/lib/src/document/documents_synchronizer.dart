...
final _logger = Logger('DocumentsSynchronizer');

//ignore: one_member_abstracts
abstract interface class DocumentsSynchronizer {
  const factory DocumentsSynchronizer(
    CatalystDatabase db,
    SignedDocumentDataSource local,
...
final class DocumentsSynchronizerImpl implements DocumentsSynchronizer {
  final CatalystDatabase _db;
  final SignedDocumentDataSource _local;
  final DocumentDataRemoteSource _remote;

  const DocumentsSynchronizerImpl(
    this._db,
    this._local,
    this._remote,
  );
  ...
  Future<DocumentsSyncResult> _sync({
    ...
    do {
      final index = await _remote.index(
        page: page,
        limit: batchSize,
        filters: filters,
      );
  ...
  Future<List<SignedDocumentRef>> _syncFilterRefs(
    ...
    final cachedRefs = await _local.filterExisting(ids);
    ...
    return refs.map(
      (ref) {
        return pool.withResource(() {
          return _remote
              .get(ref)
              .then<Result<DocumentDataWithArtifact, RefSyncException>>(
                (document) {
                  return document != null
                      ? Success(document)
                      : Failure(RefSyncException(ref, source: DocumentNotFoundException(ref: ref)));
                },
              )
              .onError((error, stack) {
  ...
  Future<DocumentsSyncResult> _syncSaveBatchResults(
    ...
    if (documents.isNotEmpty) {
      await _local.saveAll(documents);
    }
