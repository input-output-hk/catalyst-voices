...
final _logger = Logger('WorkspaceCubit');
...
final class WorkspaceCubit extends Cubit<WorkspaceState>
  ...
  WorkspaceCubitCache _cache = const WorkspaceCubitCache();
  ...
  void changeFilters({WorkspaceFilters? workspaceFilter, WorkspacePageTab? tab}) {
    _cache = _cache.copyWith(workspaceFilter: workspaceFilter, activeTab: Optional(tab));
    emit(
      state.copyWith(userProposals: state.userProposals.copyWith(currentFilter: workspaceFilter)),
    );
  ...
  Future<void> deleteProposal(DraftRef ref) async {
    try {
      emit(state.copyWith(isLoading: true));
      await _proposalService.deleteDraftProposal(ref);

      // Remove proposal from cache and rebuild state
      _removeProposalFromCache(ref);
  ...
  Future<void> exportProposal({
    required DocumentRef ref,
    required String prefix,
  }) async {
    try {
      final docData = await _proposalService.getProposalDetail(id: ref);
      ...
      final filename = '${prefix}_${ref.id}';
  ...
  Future<void> forgetProposal(DocumentRef ref) async {
  ...
  Future<void> getTimelineItems() async {
  ...
  Future<void> importProposal(Uint8List proposalData) async {
    try {
      emit(state.copyWith(isLoading: true));
      final ref = await _proposalService.importProposal(proposalData);
  ...
  Future<void> init({WorkspacePageTab? tab}) async {
    await getTimelineItems();
    _resetCache(tab: tab);
    await _rebuildWorkspaceTabCountSubs();
  }

  Future<void> unlockProposal(DocumentRef ref) async {
    final proposal = _cache.proposals?.firstWhereOrNull(
      (e) => e.id == ref,
    );
    if (proposal == null || proposal.id is! SignedDocumentRef) {
    ...
    emitSignal(OpenProposalBuilderSignal(ref: ref));
  }
  ...
  void _handleDataChange(Page<UsersProposalOverview> page) {
    ...
    if (_cache.activeTab == WorkspacePageTab.proposals) {
      _cache = _cache.copyWith(proposals: Optional(page.items));
      ...
      _cache = _cache.copyWith(userProposalInvites: Optional(page.items));
          ...
          (page) => page.map(
            (data) {
              ...
              final fundNumber = category != null
                  ? campaigns
                        .firstWhere((campaign) => campaign.hasCategory(category.id.id))
                        .fundNumber
                  : 0;
              final fromActiveCampaign = activeCampaign?.fundNumber == fundNumber;
