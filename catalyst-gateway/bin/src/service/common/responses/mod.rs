...
use types::retry_after::{RetryAfter, RetryAfterHeader};
...
impl<T> WithErrorResponses<T> {
        ...
        match err {
            err if err.is::<bb8::RunError<tokio_postgres::Error>>() => {
                ...
                WithErrorResponses::Error(ErrorResponses::ServiceUnavailable(
                    Json(error),
                    Some(RetryAfter::default().to_header()),
                ))
            },
            err => {
    ...
    #[allow(dead_code)]
    pub(crate) fn rate_limit(retry_after: Option<RetryAfter>) -> Self {
        let retry_after = retry_after.unwrap_or_default();
        let error = TooManyRequests::new(None);
        WithErrorResponses::Error(ErrorResponses::TooManyRequests(
            Json(error),
            retry_after.to_header(),
        ))
    }
}
