...
impl ErrorResponses {
    ...
    pub(crate) fn unauthorized(text: String) -> Self {
        let error = Unauthorized::new(text);
    ...
    /// Its OK if we actually never call this.  Required for the API.
    /// May be generated by the ingress.
    pub(crate) fn forbidden(msg: String) -> Self {
        let error = Forbidden::new(msg, None);
...
impl<T> WithErrorResponses<T> {
    ...
    /// May be generated by the ingress.
    pub(crate) fn forbidden(msg: String) -> Self {
        WithErrorResponses::Error(ErrorResponses::forbidden(msg))
    }
