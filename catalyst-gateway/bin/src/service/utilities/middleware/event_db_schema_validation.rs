...
/// if a event DB schema version mismatch is found the existing `State`.
pub(crate) struct EventDbSchemaVersionValidation;

impl<E: Endpoint> Middleware<E> for EventDbSchemaVersionValidation {
    type Output = EventDbSchemaVersionValidationImpl<E>;

    fn transform(&self, ep: E) -> Self::Output {
        EventDbSchemaVersionValidationImpl { ep }
    }
}

/// The new endpoint type generated by the `EventDbSchemaVersionValidation`.
pub(crate) struct EventDbSchemaVersionValidationImpl<E> {
    /// Endpoint wrapped by the middleware.
    ep: E,
}

impl<E: Endpoint> Endpoint for EventDbSchemaVersionValidationImpl<E> {
    ...
    async fn call(&self, req: Request) -> Result<Self::Output> {
        ...
        if let Err(e) = EventDB::schema_version_check().await {
            tracing::error!("Schema version check error: {e}");
...
/// A function that wraps an endpoint with the `EventDbSchemaVersionValidation`.
///
...
/// `transform` attribute.
pub(crate) fn event_db_schema_version_validation(ep: impl Endpoint) -> impl Endpoint {
    ep.with(EventDbSchemaVersionValidation)
}
