...
pub(crate) fn to_new_version(doc: SignedDocBody) -> Result<SignedDocBody, anyhow::Error> {
    ...
    if doc_type_old || doc_ref_old {
        // TODO: transform the type field as the same as you will done it in the previous step,
        // but as for the JSON field TODO: rename brand_id, campaign_id and category_id to
        ...
        let meta = if let Some(json_meta) = doc.metadata() {
            let meta = catalyst_signed_doc::Metadata::from_json(json_meta.clone())?;
