...
use super::{
    cardano::{self},
    filter::{get_registration_given_stake_address, get_registration_given_vote_key, snapshot},
    response, SlotNo,
};
use crate::{
    ...
    if let Some(stake_or_voter) = lookup {
        match StakeAddressOrPublicKey::from(stake_or_voter) {
            StakeAddressOrPublicKey::Address(cip19_stake_address) => {
                ...
                return get_registration_given_stake_address(address, session, asat, page, limit)
                    .await;
            },
