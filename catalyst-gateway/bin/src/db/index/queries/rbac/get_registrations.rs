...
const QUERY: &str = include_str!("../cql/get_registrations_by_chain_root.cql");
...
pub(crate) struct GetRegistrationsByCatalystIdQuery {
    /// Registration transaction id.
    pub(crate) transaction_id: Vec<u8>,
}

impl GetRegistrationsByCatalystIdQuery {
    /// Prepares a get registrations by chain root query.
    pub(crate) async fn prepare(session: Arc<Session>) -> anyhow::Result<PreparedStatement> {
        let get_registrations_by_chain_root_query =
