...
impl PrimaryKeyQuery {
    ...
    pub(crate) async fn prepare(session: &Arc<Session>) -> anyhow::Result<PreparedStatement> {
        ...
        .await
        .inspect_err(
            |error| error!(error=%error, "Failed to prepare get TXO by stake address primary key query."),
        )
...
impl DeleteQuery {
    ...
    pub(crate) async fn prepare_batch(
        ...
        .await
        .inspect_err(
            |error| error!(error=%error, "Failed to prepare delete TXO by stake address primary key query."),
        )
        .map_err(|error| anyhow::anyhow!("{error}\n--\n{DELETE_QUERY}"))
    }
