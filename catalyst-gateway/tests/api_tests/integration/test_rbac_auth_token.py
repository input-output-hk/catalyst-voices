...
def test_invalid_rbac_auth_token(rbac_chain_factory):
    ...
    assert resp.status_code == 401, (
        f"Expected missing token prefix: {resp.status_code} - {resp.text}"
    )

    # Missing signature -> 401
    token = rbac_chain_factory(Chain.Role0).auth_token()
    ...
    assert resp.status_code == 401, (
        f"Expected missing token signature: {resp.status_code} - {resp.text}"
    )

    # Signature invalid length -> 401
    token = rbac_chain_factory(Chain.Role0).auth_token(sig="abcd")
    resp = get(lookup=None, token=token)
    assert resp.status_code == 401, (
        f"Expected invalid length signature: {resp.status_code} - {resp.text}"
    )
    ...
    assert resp.status_code == 401, (
        f"Expected invalid signature: {resp.status_code} - {resp.text}"
    )

    # Text after `catid.` is not a Catalyst ID -> 401
    token = rbac_chain_factory(Chain.Role0).auth_token(cat_id="not_a_catid")
    resp = get(lookup=None, token=token)
    assert resp.status_code == 401, (
        f"Expected invalid Catalyst ID: {resp.status_code} - {resp.text}"
    )
    ...
    resp = get(lookup=None, token=token)
    assert resp.status_code == 401, (
        f"Expected must not contain username: {resp.status_code} - {resp.text}"
    )
    ...
    resp = get(lookup=None, token=token)
    assert resp.status_code == 401, (
        f"Expected invalid Catalyst ID format: {resp.status_code} - {resp.text}"
    )

    # Catalyst ID part doesn't contain nonce -> 401
    token = rbac_chain_factory(Chain.Role0).auth_token(nonce="")
    resp = get(lookup=None, token=token)
    ...
    assert resp.status_code == 401, (
        f"Expected no role: {resp.status_code} - {resp.text}"
    )
    # Catalyst ID part contain rotation -> 401
    resp = get(lookup=None, token=f"{head}/0/0.{tail}")
    assert resp.status_code == 401, (
        f"Expected no rotation: {resp.status_code} - {resp.text}"
    )

    # Unsupported network -> 401
    token = rbac_chain_factory(Chain.Role0, "test").auth_token()
    resp = get(lookup=None, token=token)
    assert resp.status_code == 401, (
        f"Expected unsupported network: {resp.status_code} - {resp.text}"
    )

    # Unsupported subnet -> 401
    token = rbac_chain_factory(Chain.Role0, "cardano", "test").auth_token()
    resp = get(lookup=None, token=token)
    assert resp.status_code == 401, (
        f"Expected unsupported subnet: {resp.status_code} - {resp.text}"
    )
    ...
    resp = get(lookup=None, token=token)
    assert resp.status_code == 401, (
        f"Expected key not registered: {resp.status_code} - {resp.text}"
    )

    # Nonce is not in a range -> 403
    nonce = int((datetime.now(timezone.utc) - timedelta(days=30)).timestamp())
    token = rbac_chain_factory(Chain.Role0).auth_token(nonce=nonce)
    resp = get(lookup=None, token=token)
    assert resp.status_code == 403, (
        f"Expected nonce expired: {resp.status_code} - {resp.text}"
    )

    # Signature verification, not valid signature -> 403.
    sig = "a" * 64
    token = rbac_chain_factory(Chain.Role0).auth_token(sig=sig)
    resp = get(lookup=None, token=token)
    assert resp.status_code == 403, (
        f"Expected invalid signature: {resp.status_code} - {resp.text}"
    )
...
def test_valid_rbac_auth_token(rbac_chain_factory):
    def test_valid_rbac_auth_token_inner(role: Chain):
        token = rbac_chain_factory(role).auth_token()

        resp = get(lookup=None, token=token)
        assert resp.status_code == 200, (
            f"Expected valid token that already registered: {resp.status_code} - {resp.text}"
        )
        ...
        assert resp.status_code == 200, (
            f"Expected valid token that already registered when X-API-Key does not match {resp.status_code} - {resp.text}"
        )
        ...
        assert resp.status_code == 200, (
            f"Expected valid token that already registered when X-API-Key does match, nonce validation is skipped: {resp.status_code} - {resp.text}"
        )
