...
def test_rbac_endpoints(rbac_chain_factory):
    ...
    assert resp.status_code == 200, (
        f"Expected registered stake address: {resp.status_code} - {resp.text}"
    )
    ...
    assert resp.status_code == 412, (
        f"Expected wrong format stake address: {resp.status_code} - {resp.text}"
    )
    ...
    assert resp.status_code == 200, (
        f"Expected registered cat id: {resp.status_code} - {resp.text}"
    )

    cat_id = rbac_chain_factory(Chain.Role0).short_cat_id()
    resp = get(lookup=cat_id, token=auth_token)
    assert resp.status_code == 200, (
        f"Expected registered short cat id: {resp.status_code} - {resp.text}"
    )
    ...
    resp = get(lookup=cat_id, token=auth_token)
    assert resp.status_code == 404, (
        f"Expected not registered cat id: {resp.status_code} - {resp.text}"
    )
    ...
    resp = get(lookup="cat_id", token=auth_token)
    assert resp.status_code == 412, (
        f"Expected wrong format cat id: {resp.status_code} - {resp.text}"
    )
