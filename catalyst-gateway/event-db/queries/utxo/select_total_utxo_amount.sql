...
SELECT cardano_txn_index.network, SUM(cardano_utxo.value) as total_utxo_amount

FROM cardano_utxo

INNER JOIN cardano_txn_index
    ON cardano_txn_index.id = cardano_utxo.tx_id

INNER JOIN cardano_slot_index
    ON cardano_slot_index.slot_no = cardano_txn_index.slot_no
