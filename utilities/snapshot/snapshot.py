...
def snapshot(
    ...
    url = f"{host}/api/v1/cardano/registration/cip36?lookup=ALL&limit={page_limit}&asat=SLOT:{slot_no}"
    ...
    try:
        ...
        pprint("Processing registrations..!")
    ...
    with open("snapshot_errors.json", "w") as outfile:
        json.dump(snapshot_errors, outfile, indent=2)
...
def process_chunk(
    ...
    """
    for chunk in chunk_of_registrations:

        # Voter may have multiple registration
        for registration in chunk["registrations"]:

            latest_stake_pub_key = registration["stake_pub_key"]

            # latest regsistration slot no to query staked ada
            slot_num = registration["slot_no"]
            ...
            if stake_address == None:
                snapshot_errors.append(registration)
                continue

            url = f"{host}/api/v1/cardano/assets/{stake_address}?&asat=SLOT:{slot_num}"
            headers = {"Authorization": bearer_token, "X-API-Key": api_key}

            response = requests.get(url, headers=headers)
            if response.status_code != 200:
                # stake address has no staked ada. Assign -1.
                registration["voting_power"] = -1

            else:
                json_data = json.loads(response.text)
                # Append voting power field
                registration["voting_power"] = json_data["persistent"]["ada_amount"]

            # Add stake address field
            registration["stake_addr"] = stake_address

            snapshot_data.append(registration)
