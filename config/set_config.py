...
def set_config(env: str):
    ...
    env_dir = os.path.join(repo_root, "config", env)
    ...
    print(f"Applying default config: {config}")
    # TODO: Apply config to endpoint
    ...
    for ip_config_path in ip_config_paths:
        ip_config = load_json_file(ip_config_path)
        print(f"Applying IP-specific config from {os.path.basename(ip_config_path)}: {ip_config}")
        # TODO: Apply IP-specific config to endpoint

    print(f"Configuration for '{env}' completed successfully.")
