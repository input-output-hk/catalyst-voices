import { cY as abs, cV as subtract, d2 as ITxBalanceType, eP as add, cX as divide, bi as getTimestampFromSlot, c$ as date_default, B as useFormatter } from "./index.js";
...
const { formatTxType } = useFormatter();
  ...
  for (const reward of rewardList) {
    ...
    for (let i = 0; i < numFields; i++) {
      ...
      switch (field) {
        ...
        case "Koinly Date":
          if (!useLocalTimezone) {
            csv += date_default.formatDate(reward.tsSpendable, "YYYY-MM-DD HH:mm:ss");
          } else {
            csv += '"' + formatDatetime(reward.tsSpendable, true, true, true) + '"';
          }
          break;
        case "Sent Amount":
          break;
